...
describe('Selenium Demo Test Suite', function () {
    ...
    it('should fail to search for ChemDraw at bing.com', function () {
        let Url: string = "http://www.bing.com";
        return driver.get(Url).then(() => {
            console.log(`Page "${Url}" opened`);
        }).then(() => {
            return driver.getCurrentUrl().then((currentUrl) => {
                ...
                searchBox.sendKeys('ChemDraw');
                return searchBox.getAttribute('value').then((value) => {
                    value.should.equal('ChemDraw');
                });
            });
        });
    });

    it('should search for ChemDraw at bing.com', function () {
        let Url: string = `http://www.bing.com`;
